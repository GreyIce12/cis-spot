...
interface newValue {
  map(arg0: (news: any) => JSX.Element): React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactNode
...
export default function Innovations({value}:Value) {
  ...
  <div className='flex flex-wrap justify-center items-center space-x-4' >
    ...
    { value.map((news) => {
    return(
<div key={news.key} className="max-w-sm bg-white border p-20 m-20 top-6 border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
    <a href={news.link}>
...
export async function getStaticProps() {
	const value = await fetch('http://localhost:3000/api/techno');
	...
	return {
		props: {
       value :(await value.json()) as Value	
		}
