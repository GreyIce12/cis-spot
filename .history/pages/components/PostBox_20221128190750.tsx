...
const [votes, setVotes] = useState<boolean>()
  ...
  const upVote = async (isUpvote: boolean) =>{
    ...
    if(!session){ return(toast("Sign in Please ! "))}

    if(votes && isUpvote) return;
    if(votes === false && !isUpvote) return;
  ...
  useEffect(() =>{
    ...
    const nvote = votes?.find(
     (votes) => votes.username == session?.user?.name)?.upvote
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-start space-y-1 rounded-l-md bg-gray-50 p-4 text-gray-400">
        <ArrowUpwardIcon onClick={() => upVote(true)} className={`voteButtons hover:text-green-400${votes && 'text-green-400'}`} />
        <p className="text-xs font-bold text-black">0</p>
        <ArrowDownwardIcon onClick={() => upVote(false)} className={`voteButtons hover:text-red-400${votes == false && 'text-red-400'}`} />
        {/* Votes */}
      </div>
