...
export default function Innovations({value}:Value) {
  ...
  const tile_truncate = (input:any) =>
  input?.length > 300 ? `${input.substring(0, 20)}...` : input; 
  return (
  ...
  <div className='flex flex-wrap justify-center items-center space-x-2' >
    ...
    return(
<div key={news?.key} className="max-w-sm bg-white border p-10 m-20 top-6 border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
    <a href={news?.link}>
        <img className="rounded-t-lg" src={news?.media} alt="" />
    </a>
    <div className="p-5">
        <a href={news?.clean_url}>
            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{truncate (news?.title)}</h5>
        </a>
